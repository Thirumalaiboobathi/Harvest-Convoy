...
- [ADR-013](docs/adr/ADR-013-route-proposal-and-operator-override.md) — operator route proposal with Accept/Modify/Drop (Part 1), proxy registration for phone-less farmers via `/addfarmer`/`/linkfarmer` with a bounded-window undo (Part 2), and a farmer's own read-only "Why?" answer on a not-ready or lost-escalation message (Part 3)
- [ADR-014](docs/adr/ADR-014-help-command.md) — `/help`, a read-only status command for a registered farmer or the cluster's operator, the one deliberate exception to "agent speaks first or a bounded reply"
